# WAA_POS_Pro_v5_2026.py
# Complete POS system - Updated payment methods with Faysal & Meezan Bank
...
with tab_sale:
    ...
    with col1:
        search = st.text_input("Search Item", placeholder="Type name or leave empty for all items...")
    with col2:
    ...
    if not items_df.empty:
        ...
        selected = st.radio("Select Item", options, index=None)
...
# ── PAYMENTS (Updated with Faysal & Meezan) ────────────────
with tab_payments:
    ...
    if not customers:
        ...
        amt = st.number_input("Amount Received (Rs)", 0, step=500)
...
with tab_returns:
    ...
    if cust_list:
        ...
        item = st.selectbox("Item", [r[0] for r in c.execute("SELECT item FROM inventory")])
        qty = st.number_input("Quantity", 1)
        amt = st.number_input("Return Amount", 0)
...
st.sidebar.caption("WAA POS Pro • v5 • Updated: Payment Methods • 2026")
